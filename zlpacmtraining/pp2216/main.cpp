...
int x[1001][1001],X[1001][1001];//行的最小最大值
int y[1001][1001],Y[1001][1001];//列的最小最大值
    ...
    for (int I=1;I<=n;I++)
        {
            ...
            for (int i=2;i<=m;i++)
                {
                    while (a[I][i]>=a[I][Q[BACK]]&&FRONT<=BACK) BACK--;//满足递减，最大值
                    while (a[I][i]<=a[I][q[back]]&&front<=back) back--;//满足递增。最小值
                    BACK++;back++;Q[BACK]=i;q[back]=i;
                    while (i-Q[FRONT]>=k) FRONT++;//超过了，就出队第一个，即第一个往后移
                    while (i-q[front]>=k) front++;
                    if (i>=k) X[I][i-k+1]=a[I][Q[FRONT]],x[I][i-k+1]=a[I][q[front]];
                }
        }
    for (int I=1;I<=m-k+1;I++)//列在变，这里I是列
        {
            FRONT=BACK=front=back=Q[1]=q[1]=1;
            for (int i=2;i<=n;i++)//i是行，一列一列的比较，所以比较列的每一行
                {
